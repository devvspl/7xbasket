...
<section class="bg-[#f0faf4] py-10 text-center">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p class="text-gray-500 text-base max-w-lg mx-auto">Have questions about our franchise? Our team is ready to help you get started.</p>
...
<section class="py-10 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            @foreach([
                ['📧', 'Email', 'info@7xbasket.com', 'mailto:info@7xbasket.com'],
                ['📞', 'Phone', '+91 9870275327', 'tel:+919870275327'],
                ['📍', 'Address', 'Lajpat Nagar, New Delhi', 'https://maps.app.goo.gl/xKx7arLWfbyjaZvB8'],
                ['🕐', 'Hours', 'Mon–Sat: 9AM – 6PM', null],
            ] as [$icon, $label, $value, $href])
            <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex items-start gap-3 hover:shadow-md transition-shadow">
                <div class="w-10 h-10 bg-[#f0faf4] rounded-xl flex items-center justify-center text-xl flex-shrink-0">{{ $icon }}</div>
                <div class="min-w-0">
                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">{{ $label }}</p>
                    @if($href)
                    <a href="{{ $href }}" {{ str_starts_with($href, 'http') ? 'target="_blank" rel="noopener"' : '' }}
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm">
                ...
                <p class="text-gray-400 text-sm mb-6">Fill in your details and we'll get back to you within 24 hours.</p>
                ...
                <form id="contactPageForm" action="{{ route('apply.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="store_area" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#109125] focus:ring-1 focus:ring-[#109125] transition-all bg-white">
                        ...
                        <div>
                            ...
                            <select name="property_type" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#109125] focus:ring-1 focus:ring-[#109125] transition-all bg-white">
                    ...
                    <div>
                        ...
                        <select name="opening_timeline" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#109125] focus:ring-1 focus:ring-[#109125] transition-all bg-white">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        .then(json => {
            ...
            if (json.success) {
                msg.className = 'rounded-xl px-4 py-3 text-sm font-medium mb-5 bg-green-50 text-green-700 border border-green-200';
                msg.textContent = json.message;
                form.reset();
                btn.textContent = 'Submitted ✓';
            } else {
                msg.className = 'rounded-xl px-4 py-3 text-sm font-medium mb-5 bg-red-50 text-red-700 border border-red-200';
